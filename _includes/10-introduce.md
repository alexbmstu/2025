...
| void **mq_send64**(unsigned long long data)                  | Синхронная передача 8 байт (аналогично mq_send) в gpc.                                                                                                                                                                   |
| void **mq_send128**(uint128_t data)                          | Синхронная передача 16 байт (128 бит) в gpc.                                                                                                                                                                             |
...
| uint64_t **mq_receive**()                                    | Синхронное получение 8 байт из gpc. Возвращает данные типа unsigned long long.                                                                                                                                           |
| uint64_t **mq_receive64**()                                  | Синхронное получение 8 байт из gpc (аналогично mq_receive).                                                                                                                                                              |
| uint128_t **mq_receive128**()                                | Синхронное получение 16 байт (128 бит) из gpc.                                                                                                                                                                           |
...
Методы mq_send128 и mq_receive128 обеспечивают работу с 128-битными значениями, что особенно важно при передаче составных ключей для ассоциативной памяти микропроцессора. Эти методы позволяют передавать сложные структуры данных, содержащие несколько полей, за одну операцию, что значительно повышает эффективность обмена между хост-системой и GPC.

Методы mq_send64 и mq_receive64 являются синонимами базовых методов работы с 64-битными значениями и предоставляются для обеспечения единообразия API и обратной совместимости с кодом, который может явно использовать эти названия.

Асинхронные версии методов mq_send и mq_receive с параметром bufsize позволяют эффективно передавать большие объемы данных без блокировки основного потока выполнения. Возвращаемый указатель на объект std::thread позволяет контролировать выполнение операции - ожидать ее завершения с помощью метода join() или отслеживать состояние передачи. Важной особенностью является гарантированная последовательность выполнения нескольких асинхронных операций в порядке их запуска, что обеспечивает целостность данных при сложных сценариях взаимодействия.
...
| void **mq_send64** (unsigned long long data);                 | Передача 8 байт (аналогично mq_send) в хост-подсистему                                         |
| void **mq_send128** (uint128_t data);                         | Передача 16 байт (128 бит) в хост-подсистему                                                   |
| void **mq_send_flush** (unsigned long long data);             | Немедленная отправка 8 байт с принудительным сбросом буфера в хост-подсистему                  |
| void **mq_send_flush128** (uint128_t data);                   | Немедленная отправка 16 байт (128 бит) с принудительным сбросом буфера в хост-подсистему       |
...
| unsigned long long **mq_receive64**();                       | Получение 8 байт из хост-подсистемы (аналогично mq_receive)                                    |
| uint128_t **mq_receive128**();                               | Получение 16 байт (128 бит) из хост-подсистемы                                                 |
...
Функции mq_send128 и mq_receive128 обеспечивают работу с 128-битными значениями, что особенно важно для обработки сложных ключей в ассоциативной памяти микропроцессора. Эти функции позволяют передавать составные ключи, содержащие несколько полей, за одну операцию, что значительно повышает эффективность обмена данными между хост-системой и микропроцессором. Например, при работе со структурой users::key, которая содержит поля user и idx, передача всего ключа осуществляется одним вызовом mq_send128.
...
Функции mq_send_flush и mq_send_flush128 обеспечивают немедленную отправку данных с принудительным сбросом буферов. Это важно в сценариях, где требуется минимальная задержка между отправкой данных и их обработкой на стороне хоста. В отличие от обычных функций mq_send, которые могут буферизовать данные для пакетной передачи, функции с суффиксом flush гарантируют немедленную отправку, что особенно важно для синхронизации и передачи данных.

Функции mq_send64 и mq_receive64 являются синонимами базовых функций работы с 64-битными значениями и предоставляются для обеспечения единообразия API. Все функции работы с очередями сообщений построены по принципу минимальной задержки и максимальной пропускной способности, что позволяет эффективно использовать их в высокопроизводительных сценариях обработки графовых данных и ассоциативных запросов.
...
В библиотеке lnh64 l0 реализованы шаблоны для работы со структурами составных ключей. Обязательным требованием к ним является общий размер, который должен быть равен 128 разрядам для представления значимых 100 бит ключа. Для представления значение необходимо соблюдать общий размер полей структуры равный 64 битам.
...
```c
 ...
 STRUCT128(A_key)
 {
        ...
        uint32_t    nu   :28;   //Не используется 
 };
...
## 3.7. Пример 4. Обработка ключей и значений большого размера 

При обработке данных реальных задач необходимо формировать коллекции анных на основе большого количества полей, в том числе строковой информации. Для реализации этой возможности разработана программная библиотека для работы с длинными ключами и значениями, размер которых может составлять до 4КБ. В данном разделе рассматривается ряд программных решений для обработки таких данных: от пересылки ключей и значений между хост и sw_kernel, до обработки json объектов. Далее показано использование объектов MessageProcessor в обоих микропроцессорах: микропроцессоре хост-подсистемы (x86) и ускорительной карте (riscv64). 

Возможности библиотеки продемонстированы ниже на примере программной системы (пример 4\ @BibliotekaPrimerovRazrabotki),  представляющей собой комплексное решение для управления пропусками на транспортные средства, построенное на гетерогенной архитектуре с использованием специализированного микропроцессора RISC-V. Логика работы системы основана на разделении обязанностей между хост-подсистемой, выполняющей функции интерфейса и управления, и программным ядром на микропроцессоре, оптимизированным для эффективного хранения и поиска данных. Основная задача системы — регистрация студентов, выдача им пропусков на различные типы транспорта и последующая проверка действительности этих пропусков по запросам.

Для формирования единого описания структур создаются объекты для взаимодействия, и передаются длинные ключи и значения (структуры описаны в файле common_struct.h в папке include. Переданный ключ изменяется в программном ядре и посылается обратно. В файле common_struct.h определяются ключевые структуры данных, которые используются во всей системы. Структура students_Key содержит идентификатор студента и его полное имя, выровненное по правому краю для оптимизации хранения в ассоциативной памяти. Поле name фиксированного размера в 128 байт обеспечивает единообразие структуры данных, а инициализация нулями гарантирует предсказуемое поведение при операциях сравнения. Структура students_Value намеренно оставлена пустой, так как в данной реализации вся необходимая информация о студенте содержится в ключе.

**Листинг: Объявление общих структур данных**

```c
// Объявляем enum с типами транспортных средств
typedef enum {
    SCOOTER,    // Самокат
    MOTORCYCLE,  // Мотоцикл
    CAR         // Автомашина
...
RALIGNED_STRUCT (students_Key,
	uint64_t 	student_id = 0;
	char 		name[128] = {0,};
)

LALIGNED_STRUCT (students_Value,
    //не используется
)

// parking_permits: permit_id, student_id, valid_until.

RALIGNED_STRUCT (parking_permits_Key, 
	uint64_t 	permit_id = 0;
	uint64_t 	student_id = 0;
)

LALIGNED_STRUCT (parking_permits_Value,
	time_t		valid_until;
	VehicleType transportation_mode = SCOOTER;
)

```

Для управления пропусками используется структура parking_permits_Key, состоящая из идентификатора пропуска и идентификатора студента, что позволяет однозначно связывать пропуска с конкретными владельцами. В структуре parking_permits_Value хранится важная информация о сроке действия пропуска и типе транспортного средства. Использование типа time_t для поля valid_until обеспечивает совместимость со стандартными функциями работы со временем в языке C, а перечисление VehicleType строго ограничивает допустимые типы транспорта тремя вариантами: самокат, мотоцикл и автомобиль. Такая организация данных позволяет эффективно выполнять запросы на проверку действительности пропусков, сочетая поиск по идентификатору студента и анализу временных меток.

Хост-программа начинает работу с инициализации ядра GPC и загрузки программного обеспечения на микропроцессор. Этот процесс включает выделение ресурсов специализированного процессора через механизм управления драйвером и загрузку скомпилированного кода прошивки в память устройства. После успешной инициализации хост создает процессоры сообщений для взаимодействия со структурами данных студентов и пропусков, что абстрагирует прикладной код от низкоуровневых деталей обмена данными.

**Листинг: Инициализация и загрузка программного ядра**

```c
gpc *gpc64_inst = new gpc();
printf("Open gpc on %s\n", gpc64_inst->gpc_dev_path);
if (gpc64_inst->load_swk(argv[1])) {
    printf("Rawbinary %s file error\n", argv[1]);
    delete gpc64_inst;
    return -1;
}
...
Далее в примере генерируются тестовые данных. Программа использует предопределенные массивы имен, фамилий и отчеств для создания реалистичных ФИО студентов, демонстрируя способность системы работать с большими объемами структурированных данных. Для каждого студента создаются пропуска на все типы транспортных средств со случайными сроками действия, что моделирует реальные условия эксплуатации системы. Особое внимание уделяется преобразованию строковых данных: функция reverse_string обеспечивает корректное представление строк в памяти микропроцессора с учетом особенностей порядка байт, что  важно для последующего поиска и сравнения. 


После заполнения базы данных система выполняет автоматическую проверку целостности всех записей. Этот процесс включает последовательную проверку каждого созданного пропуска, сравнение фактических данных с ожидаемыми значениями и детальное логирование всех обнаруженных расхождений. Такой подход обеспечивает высокую надежность системы и позволяет выявлять потенциальные проблемы на ранних этапах разработки и тестирования.

Интерактивная часть системы предоставляет пользователю возможность проверки конкретных пропусков путем ввода ФИО студента, выбора типа транспортного средства и указания даты проверки. Введенные данные преобразуются во внутренний формат, включая реверс строки имени для соответствия формату хранения в микропроцессоре, и отправляются в виде структурированного запроса. Ответ от микропроцессора содержит информацию о наличии пропуска, необходимости его обновления и точной дате истечения срока действия, которая форматируется для удобного представления пользователю.

**Листинг: Обработка пользовательского запроса**

```c
gpc64_inst->start(__event__(ask_permit));
students.send_sync(sk, sk.size(), sv, sizeof(students_Value));
parking_permits.send_sync(ppk, ppk.size(), ppv, ppv.size());

students.receive_sync(sk, sv);
bool permit_found = gpc64_inst->mq_receive();
bool require_permit_renew = gpc64_inst->mq_receive();
time_t expire_date = gpc64_inst->mq_receive();
```

Программное ядро на микропроцессоре RISC-V построено вокруг главного цикла обработки событий, который постоянно ожидает команды от хоста. После инициализации ядра Leonhard процессор переходит в состояние готовности к выполнению операций. При получении события ядро определяет его тип и вызывает соответствующий обработчик, что обеспечивает модульность и расширяемость системы.

**Листинг: Главный цикл обработки событий в sw_kernel**
...
int main(void) {
    ...
    for (;;) {
        ...
        event_source = wait_event();
        switch(event_source) {
            case __event__(insert_student) : insert_student(); break;
            case __event__(insert_permit) : insert_permit(); break;
            case __event__(ask_permit) : ask_permit(); break;
        }
        set_gpc_state(READY);
    }
}
...
В начале программы создаются четыре экземпляра шаблонного класса MessageProcessor, специализированные для работы со структурами данных студентов и пропусков. Первые два процессора предназначены для основных операций с данными студентов, а следующие два - для работы с пропусками. Использование отдельных процессоров для запрашиваемых данных позволяет эффективно разделять потоки информации и избегать конфликтов при параллельной обработке. Номера структур, передаваемые в конструктор, соответствуют конкретным таблицам в ассоциативной памяти микропроцессора, что обеспечивает корректное размещение данных.

Обработчик insert_student реализует логику добавления записей о студентах в базу данных. Он получает данные из хост-системы через метод receive, который автоматически выполняет десериализацию ключа и значения из формата, переданного через очередь сообщений. После успешного получения данных выполняется операция insert, которая размещает информацию в ассоциативной памяти микропроцессора. Аналогичным образом работает обработчик insert_permit, который отвечает за добавление записей о транспортных пропусках. Оба обработчика демонстрируют простоту и эффективность подхода, когда сложные операции работы с данными инкапсулированы в методы класса MessageProcessor.

**Листинг: Обаботчики вставки данных**

```c

MessageProcessor<students_Key,students_Value> students(1);
MessageProcessor<students_Key,students_Value> student_reguested(1);
MessageProcessor<parking_permits_Key,parking_permits_Value> parking_permits(2);
MessageProcessor<parking_permits_Key,parking_permits_Value> parking_permits_requested(2);


void insert_student() {
        //Получение Key,Value из хост-подсистемы
        students.receive();
        students.insert();
}
...
void insert_permit() {
        //Получение Key,Value из хост-подсистемы
        parking_permits.receive();
        parking_permits.insert();

}
```

Наиболее сложная логика реализована в обработчике ask_permit, который выполняет проверку действительности пропуска. Этот обработчик начинает работу с получения запрошенных данных о студенте и пропуске через соответствующие процессоры сообщений. Ключевой особенностью является алгоритм поиска, который использует комбинацию методов работы с ассоциативной памятью для эффективного нахождения нужных записей. Сначала выполняется поиск студента по имени с использованием метода nsm (next same) с указанием смещения поля student_id. Это позволяет найти все записи, связанные с конкретным студентом, даже если точный идентификатор изначально неизвестен.


**Листинг: Обработчик запросов в SPE**

```c
void ask_permit () {
        //Получение Key,Value c name, transportation_mode и valid_unitl из хост-подсистемы
        student_reguested.receive();
        parking_permits_requested.receive();
        //Записываем поля в курсор
        students.key(student_reguested.key()); //копирование полученных полей name, transportation_mode в курсор students
        students.key().student_id=student_reguested.key().student_id=-1ull;
        bool require_permit_renew = true;
        bool permit_found = false;
        time_t t_now = parking_permits_requested.value().valid_until;
        time_t expire_date;
        if (students.nsm(FIELD_U64_ROFFSET(students_Key,student_id)) &&
              streq(student_reguested.key().name,students.key().name, sizeof(students.key().name))) {
                parking_permits.key().student_id = students.key().student_id; //Записываем найденное поле student_id во второй курсор
                parking_permits.key().permit_id = -1ull;
                while (         parking_permits.nsm(FIELD_U64_ROFFSET(parking_permits_Key,permit_id)) && 
                                parking_permits.key().student_id == students.key().student_id) 
                {
                       if (parking_permits.value().transportation_mode == parking_permits_requested.value().transportation_mode) {
                                permit_found = true;
                                require_permit_renew = require_permit_renew && is_expiring_soon(t_now, parking_permits.value().valid_until);
                                expire_date = parking_permits.value().valid_until;
                       } 
                } 
                students.send(); //пример передачи k-v
        } else {
                student_reguested.send();
        }
        mq_send(permit_found); //найден ли пропуск
        mq_send(require_permit_renew); //нужно ли обновлять пропуск
        mq_send(expire_date); //срок действия пропуска если найден
}

```

После успешного нахождения студента система выполняет проверку соответствия имени с использованием функции streq, которая сравнивает строки с учетом их специального формата хранения в микропроцессоре. После подтверждения соответствия имени система переходит к поиску соответствующих пропусков, используя аналогичный механизм nsm для обхода всех записей пропусков, связанных с найденным идентификатором студента.


В процессе поиска пропусков система проверяет соответствие типа транспортного средства и анализирует срок действия каждого найденного пропуска. Функция is_expiring_soon определяет, истекает ли срок действия пропуска в ближайшие 30 дней, что позволяет proactively уведомлять о необходимости продления. Логика обработки предусматривает учет всех найденных пропусков для данного типа транспорта, устанавливая флаг require_permit_renew в true только если все соответствующие пропуски требуют обновления.

Завершающим этапом работы обработчика является отправка результатов проверки в хост-систему. Система отправляет три ключевых параметра: флаг наличия пропуска, флаг необходимости обновления и дату истечения срока действия. В случае если студент не был найден или не было найдено соответствующих пропусков, система отправляет исходные данные обратно в хост-систему, что позволяет реализовать дополнительную логику обработки таких случаев на стороне хоста.

Архитектура системы демонстрирует эффективное разделение обязанностей между компонентами: хост-система обеспечивает удобный интерфейс и управление данными, в то время как специализированный микропроцессор выполняет ресурсоемкие операции поиска и фильтрации. Использование ассоциативной памяти позволяет достичь высокой производительности при работе с большими объемами данных, а четко определенные структуры данных обеспечивают согласованность между компонентами системы. Такой подход особенно эффективен для приложений, требующих интенсивных операций поиска и реального времени обработки запросов.


## 3.8. Пример 4. Обработка ключей и значений большого размера (версия для языка Python)

Для поддержик языка Python разработана специальныя библиотека gpc64io, которая устанавливается на целевой платформе следующей командой:

```bash
pip install gpc64io
```
 
С испольщованием данной библиотеки становится возможным реализовать все аналогичные действия хост-подсистемы с испольованием синтаксиса Python (версии 3).

Ниже рассмотре Python-программа, которая представляет собой альтернативную реализацию системы управления студенческими пропусками, использующую Jupyter Notebook в качестве среды выполнения и специализированную библиотеку gpc64io для взаимодействия с микропроцессором RISC-V. Этот подход демонстрирует современный способ работы с гетерогенными системами через высокоуровневый Python-интерфейс, что значительно упрощает разработку и отладку по сравнению с традиционными C++ решениями.

Программа начинается с импорта необходимых библиотек, включая стандартные модули Python для работы со случайными числами, временем и типами данных, а также специализированный модуль gpc64io.base, который предоставляет класс GPC для абстракции взаимодействия с аппаратным ускорителем. Важной особенностью является использование Jupyter Notebook, который позволяет выполнять код инкрементально, что особенно удобно для отладки и исследования работы системы.

Инициализация системы начинается с создания экземпляра класса GPC, который автоматически находит и резервирует доступное ядро микропроцессора (листинг\ @lst:5.17). После успешного подключения программа загружает программное ядро (sw_kernel) из файла rawbinary, который содержит скомпилированный код для выполнения на RISC-V. Особое внимание уделяется загрузке метаданных: программа читает файл gpc_handlers.h для получения информации об обработчиках событий и common_struct.h для извлечения определений структур данных. Это позволяет динамически создавать Python-объекты, соответствующие структурным типам данных, определенным в C++, что обеспечивает типобезопасность и удобство работы.


**Листинг: Инициализация GPC и загрузка метаданных**

```python
gpc = GPC()
swk_path = str(pth.Path().absolute()/"sw-kernel/sw_kernel.rawbinary")
...
Поскольку микропроцессор использует особый формат хранения строк с обратным порядком байт (little-endian reversed), программа включает специализированные функции преобразования. Функция string_to_little_endian_reversed преобразует обычные строки Python в формат, понятный микропроцессору, а little_endian_reversed_to_string выполняет обратное преобразование. Эти функции обеспечивают корректную передачу ФИО студентов между хост-системой и микропроцессором (листинг\ @lst:5.18).

**Листинг: Функции преобразования строковых данных**

```python
def string_to_little_endian_reversed(input_str, max_length=40):
    if not isinstance(input_str, bytes):
        input_bytes = input_str.encode('utf-8')[:max_length]
    else:
        input_bytes = input_str[:max_length]
    reversed_bytes = input_bytes[::-1]
    return reversed_bytes[:max_length]

def little_endian_reversed_to_string(byte_array):
    if hasattr(byte_array, 'raw'):
        raw_bytes = bytes(byte_array.raw)
    else:
        raw_bytes = bytes(byte_array)
    restored_bytes = raw_bytes[::-1]
    return restored_bytes.decode('utf-8', errors='replace')
```

Генерация тестовых данных реализована через создание всевозможных комбинаций имен, фамилий и отчеств из предопределенных списков (листинг\ @lst:5.19). Для каждого студента создается уникальный идентификатор, и для каждого типа транспортного средства генерируется пропуск со случайной датой истечения срока действия. Все данные сохраняются в словаре generated_data для последующей проверки целостности. Процесс вставки данных в микропроцессор осуществляется через вызовы start_handler с последующей отправкой структур данных через mq_send_struct.

**Листинг: Генерация и отправка тестовых данных**

```python
for i in range(NAME_COUNT):
    for j in range(MIDDLE_COUNT):
        for k in range(LASTNAME_COUNT):
            name_buf = fill_name_full(first_names[i], middle_names[j], last_names[k])
            student_key.name = string_to_little_endian_reversed(name_buf, NAME_LENGTH)
            student_id = i * LASTNAME_COUNT * MIDDLE_COUNT + j * LASTNAME_COUNT + k + 1
            
            # Отправка данных студента
            gpc.start_handler("insert_student")
            gpc.mq_send_struct(student_key)
            gpc.mq_send_struct(student_value)
```

Функция verify_all_permissions выполняет автоматическую проверку целостности всех записей в системе (листинг\ @lst:5.20). Для каждого студента и каждого его пропуска программа отправляет запрос к микропроцессору и сравнивает полученные данные с ожидаемыми значениями. Проверка включает валидацию идентификаторов студентов, корректности имен и соответствия сроков действия пропусков. Детальное логирование позволяет идентифицировать конкретные проблемы в случае расхождений.

**Листинг: Функция автоматической проверки целостности данных**

```python
def verify_all_permissions():
    for student_id, student_data in generated_data.items():
        student_key.student_id = student_id
        student_key.name = student_data['name_bytes']
        
        for permit in student_data['permits']:
            parking_permit_key.student_id = student_id
            parking_permit_key.permit_id = permit['permit_id']
            parking_permit_value.transportation_mode = permit['vehicle_type']
            parking_permit_value.valid_until = permit['valid_until']
            
            # Отправка запроса и получение ответа
            gpc.start_handler("ask_permit")
            gpc.mq_send_struct(student_key)
            gpc.mq_send_struct(student_value)
            gpc.mq_send_struct(parking_permit_key)
            gpc.mq_send_struct(parking_permit_value)
            
            # Анализ результатов
            received_student_key = gpc.mq_receive_struct(structs['students_Key'])
            permit_found = gpc.mq_receive_uint64()
            require_renew = gpc.mq_receive_uint64()
            expire_date = gpc.mq_receive_uint64()
```

Интерактивная часть программы, активируемая через флаг USE_USER_REQUEST, позволяет пользователю вручную проверять пропуска путем ввода ФИО студента, выбора типа транспортного средства и указания даты проверки (листинг\ @lst:5.21). Введенные данные преобразуются во внутренний формат и отправляются в микропроцессор. Полученный ответ анализируется и форматируется для удобного представления пользователю, включая информацию о необходимости обновления пропуска и точной дате истечения срока действия.

**Листинг: Обработка пользовательского запроса**

```python
if USE_USER_REQUEST:
    name_input = input_padded_name("Введите ФИО", NAME_LENGTH)
    student_key.name = string_to_little_endian_reversed(name_input, NAME_LENGTH)
    
    print("Выберите тип транспорта:")
    for i, name in enumerate(vehicle_names, 1):
        print(f"{i}. {name}")
    parking_permit_value.transportation_mode = int(input("Ваш выбор: ")) - 1
...
По завершении работы программа освобождает ресурсы микропроцессора через вызов del(gpc). Python-реализация демонстрирует несколько преимуществ по сравнению с C++ версией: более простой и выразительный код, интерактивность выполнения, удобство отладки и возможность использования богатой экосистемы библиотек Python. При этом сохраняется вся функциональность оригинальной системы, включая высокопроизводительную обработку данных на специализированном микропроцессоре и надежную проверку целостности данных.


## 3.9. Запуск демо проекта 4 
...
```bash
Open gpc on /dev/gpc3
Rawbinary loaded from sw-kernel/sw_kernel.rawbinary

Начинаем автоматическую проверку всех отправленных пропусков...

Результаты автоматической проверки:
Всего проверок: 29997
Успешных: 29997
Неудачных: 0
Процент успеха: 100.00%
Введите ФИО (например, Иван Павлович Щукин): Иван Павлович Щукин
...
Введите дату в формате ДД.ММ.ГГГГ (например, 20.02.2026): 24.11.2025
Пользователь Иван Павлович Щукин - Обновлять пропуск не требуется, срок действия: 03.03.2026
```
...
Структура: students_Key_unaligned
Поля вложенной структуры data:
  student_id: c_ulong (8 байт)
  name: c_byte_Array_128 (128 байт)

Структура: students_Key
Поля вложенной структуры data:
  student_id: c_ulong (8 байт)
  name: c_byte_Array_128 (128 байт)

Структура: students_Value_unaligned
Поля вложенной структуры data:

Структура: students_Value
Поля вложенной структуры data:

Структура: parking_permits_Key_unaligned
Поля вложенной структуры data:
  permit_id: c_ulong (8 байт)
  student_id: c_ulong (8 байт)

Структура: parking_permits_Key
Поля вложенной структуры data:
  permit_id: c_ulong (8 байт)
  student_id: c_ulong (8 байт)

Структура: parking_permits_Value_unaligned
Поля вложенной структуры data:
  valid_until: c_ulong (8 байт)
  transportation_mode: c_byte (1 байт)

Структура: parking_permits_Value
Поля вложенной структуры data:
  valid_until: c_ulong (8 байт)
  transportation_mode: c_byte (1 байт)
```

Информация используется для создания экземпляров классов `students_Key`,`students_Value`,`parking_permits_Key` и `parking_permits_Value`. При этом возможно задавать переменную длинну полей, что удобно для объявления строковых констант переменной длинны. 
...
#Key
student_key.student_id = 0
name_input = input_padded_name("Введите ФИО (например, Иван Павлович Щукин)", NAME_LENGTH)
student_key.name = string_to_little_endian_reversed(name_input,NAME_LENGTH)
#Value
 date_input = input("\nВведите дату в формате ДД.ММ.ГГГГ (например, 20.02.2026): ")
    day, month, year = map(int, date_input.split('.'))
    parking_permit_value.valid_until = int(datetime(year, month, day).timestamp())

# Создаем запись KeyS переменной длинны с явным заданием длины
student_key = structs['students_Key']()
student_value = structs['students_Value']()
parking_permit_key = structs['parking_permits_Key']()
parking_permit_value = structs['parking_permits_Value']()
# Явное задание длины поля
student_key.size = len(name_input) + len(student_key.student_id)
```

Использование строковых полей без указания длинны влечет за собой установку максимальной длины поля, что может быть нежелеательно. Кроме того, на хранение такого поля будет затрачено больше ресурсов gpc: 
...
Заупуск проекта осуществляется в ноутбуке ex6.ipynb.
...
* Визуализация inbox на основе модулярности [Ньюмана](https://alexbmstu.github.io/2025/#442-%D0%B0%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA%D0%B0-%D1%86%D0%B5%D0%BD%D1%82%D1%80%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8).
* Визуализация на основе силового алгоритма [Фрухтерамана-Рейнгольда](https://alexbmstu.github.io/2025/#444-%D0%B0%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC%D1%8B-%D1%80%D0%B0%D1%81%D0%BA%D0%BB%D0%B0%D0%B4%D0%BA%D0%B8-%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2).
* Спиральная визуализация на основе [центральности](https://alexbmstu.github.io/2025/#442-%D0%B0%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA%D0%B0-%D1%86%D0%B5%D0%BD%D1%82%D1%80%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8).
* Спиральная матричная визуализация на основе [центральности](https://alexbmstu.github.io/2025/#442-%D0%B0%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA%D0%B0-%D1%86%D0%B5%D0%BD%D1%82%D1%80%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8).
* Визуализацию графа-решетки на основе [центральности](https://alexbmstu.github.io/2025/#442-%D0%B0%D0%BB%D0%B3%D0%BE%D1%80%D0%B8%D1%82%D0%BC-%D0%BF%D0%BE%D0%B8%D1%81%D0%BA%D0%B0-%D1%86%D0%B5%D0%BD%D1%82%D1%80%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8).
...
Ознакомиться с проектами [Примера 4](https://alexbmstu.github.io/2025/#471-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-4-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F-%D1%8F%D0%B7%D1%8B%D0%BA%D0%B0-python-%D0%B8-%D0%B1%D0%B8%D0%B1%D0%BB%D0%B8%D0%BE%D1%82%D0%B5%D0%BA%D0%B8-gpc64io-%D0%B4%D0%BB%D1%8F-%D0%BF%D1%80%D0%B8%D0%B5%D0%BC%D0%BE-%D0%BF%D0%B5%D1%80%D0%B5%D0%B4%D0%B0%D1%87%D0%B8-%D0%B4%D0%B0%D0%BD%D0%BD%D1%8B%D1%85-%D0%BC%D0%B5%D0%B6%D0%B4%D1%83-%D1%85%D0%BE%D1%81%D1%82-%D0%BF%D0%BE%D0%B4%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%BE%D0%B9-%D0%B8-sw_kernel), [Примера 5](https://alexbmstu.github.io/2025/#472-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-5-%D0%B4%D0%B5%D0%BC%D0%BE%D0%BD%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8F-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D0%BD%D0%B8%D1%8F-jupyter-%D0%BD%D0%BE%D1%83%D1%82%D0%B1%D1%83%D0%BA%D0%BE%D0%B2-%D0%B8-%D1%8F%D0%B7%D1%8B%D0%BA%D0%B0-python-%D0%B4%D0%BB%D1%8F-%D0%B2%D0%B8%D0%B7%D1%83%D0%B0%D0%BB%D0%B8%D0%B7%D0%B0%D1%86%D0%B8%D0%B8-%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2), [Примера 6](https://alexbmstu.github.io/2025/#473-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-6-%D0%B4%D0%B5%D0%BC%D0%BE%D0%BD%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8F-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F-%D0%BC%D0%B8%D0%BA%D1%80%D0%BE%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D1%81%D1%81%D0%BE%D1%80%D0%B0-%D0%BB%D0%B5%D0%BE%D0%BD%D0%B0%D1%80%D0%B4-%D1%8D%D0%B9%D0%BB%D0%B5%D1%80-%D0%B4%D0%BB%D1%8F-%D0%B0%D0%BD%D0%B0%D0%BB%D0%B8%D0%B7%D0%B0-%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2-%D0%B7%D0%BD%D0%B0%D0%BD%D0%B8%D0%B9).
...
В данном разделе рассматривается способ генерации музыкального произведения на основе графов ДеБрюйна, и его последующая стилизация. Получение графов ДеБрюйна музыкальных произведений было подробно рассмотрено в [Примере 6](https://alexbmstu.github.io/2025/#473-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-6-%D0%B4%D0%B5%D0%BC%D0%BE%D0%BD%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8F-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F-%D0%BC%D0%B8%D0%BA%D1%80%D0%BE%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D1%81%D1%81%D0%BE%D1%80%D0%B0-%D0%BB%D0%B5%D0%BE%D0%BD%D0%B0%D1%80%D0%B4-%D1%8D%D0%B9%D0%BB%D0%B5%D1%80-%D0%B4%D0%BB%D1%8F-%D0%B0%D0%BD%D0%B0%D0%BB%D0%B8%D0%B7%D0%B0-%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2-%D0%B7%D0%BD%D0%B0%D0%BD%D0%B8%D0%B9). 
...
Помимо этого можно формировать собственные графы деБрюйна с помощью кода [Примера 6](https://alexbmstu.github.io/2025/#473-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-6-%D0%B4%D0%B5%D0%BC%D0%BE%D0%BD%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8F-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F-%D0%BC%D0%B8%D0%BA%D1%80%D0%BE%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D1%81%D1%81%D0%BE%D1%80%D0%B0-%D0%BB%D0%B5%D0%BE%D0%BD%D0%B0%D1%80%D0%B4-%D1%8D%D0%B9%D0%BB%D0%B5%D1%80-%D0%B4%D0%BB%D1%8F-%D0%B0%D0%BD%D0%B0%D0%BB%D0%B8%D0%B7%D0%B0-%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2-%D0%B7%D0%BD%D0%B0%D0%BD%D0%B8%D0%B9).
...
Описание стадии 1 подробно представлено в разделе: [Примера 6](https://alexbmstu.github.io/2025/#473-%D0%BF%D1%80%D0%B8%D0%BC%D0%B5%D1%80-6-%D0%B4%D0%B5%D0%BC%D0%BE%D0%BD%D1%81%D1%82%D1%80%D0%B0%D1%86%D0%B8%D1%8F-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F-%D0%BC%D0%B8%D0%BA%D1%80%D0%BE%D0%BF%D1%80%D0%BE%D1%86%D0%B5%D1%81%D1%81%D0%BE%D1%80%D0%B0-%D0%BB%D0%B5%D0%BE%D0%BD%D0%B0%D1%80%D0%B4-%D1%8D%D0%B9%D0%BB%D0%B5%D1%80-%D0%B4%D0%BB%D1%8F-%D0%B0%D0%BD%D0%B0%D0%BB%D0%B8%D0%B7%D0%B0-%D0%B3%D1%80%D0%B0%D1%84%D0%BE%D0%B2-%D0%B7%D0%BD%D0%B0%D0%BD%D0%B8%D0%B9).
